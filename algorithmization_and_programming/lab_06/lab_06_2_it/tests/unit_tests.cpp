...
TEST(MaxIndexTest, HandlesNormalValues) {
    ...
    EXPECT_EQ(lib::Max_index(arr, size), 4); // Index of max (5) is 4
}

TEST(MaxIndexTest, HandlesAllNegativeValues) {
    int arr[] = {-1, -2, -3, -4, -5};
    int size = sizeof(arr) / sizeof(arr[0]);
    EXPECT_EQ(lib::Max_index(arr, size), 0); // Index of max (-1) is 0
}

TEST(MaxIndexTest, HandlesSingleElement) {
    int arr[] = {10};
    int size = sizeof(arr) / sizeof(arr[0]);
    EXPECT_EQ(lib::Max_index(arr, size), 0); // Index of max (10) is 0
}
...
TEST(MinIndexTest, HandlesNormalValues) {
    int arr[] = {5, 4, 3, 2, 1};
    int size = sizeof(arr) / sizeof(arr[0]);
    EXPECT_EQ(lib::Min_index(arr, size), 4); // Index of min (1) is 4
}

TEST(MinIndexTest, HandlesAllNegativeValues) {
    int arr[] = {-1, -2, -3, -4, -5};
    int size = sizeof(arr) / sizeof(arr[0]);
    EXPECT_EQ(lib::Min_index(arr, size), 4); // Index of min (-5) is 4
}

TEST(MinIndexTest, HandlesSingleElement) {
    int arr[] = {10};
    int size = sizeof(arr) / sizeof(arr[0]);
    EXPECT_EQ(lib::Min_index(arr, size), 0); // Index of min (10) is 0
}
